...
export class MFormSandbox extends Vue {
    serverResponseStatuses: number[] = [100, 200, 300, 400, 500];
    serverResponseStatus: number = 100;
    forms: Form[] = [
        new Form({
    ...
    submit(formIndex: number): void {
        if (formIndex === 12) {
            (this.$refs['form-12'] as MForm).serverResponse = new FormServerResponse(
                this.serverResponseStatus,
                `this message would be generated by the server`
            );
        }
    }
