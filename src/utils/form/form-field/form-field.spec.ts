...
let validationState: FormFieldValidation = new FormFieldValidation();
...
describe(`FormField`, () => {
    describe(`When we create a new instance with a value and a validating function`, () => {
        ...
        it(`Then the value should be observable`, (done) => {
            formField.Changes.subscribe((value: string) => {
                expect(value).toBe(NEW_FIELD_VALUE);
        ...
        afterEach(() => formField = new FormField((): string => FIELD_VALUE, VALIDATION_FUNCTION, { messageAfterTouched: false }));
    });
